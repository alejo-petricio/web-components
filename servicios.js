...
function getParamsServicios() {
    return fetch('https://cdn.contentful.com/spaces/7zcejl3en0an/environments/master/entries?access_token=5NYdT1P33L6TDQQUnP8VWEPxKGiVrg0BvgISZxCGYvU&content_type=servicios')
        ...
        .then((data) => {
            //const collection = data.items.map(function (item) {
                return {
                    ...
                    img: data.includes.Asset[1].fields.file.url,
                    sombra: data.includes.Asset[0].fields.file.url
                }
           // });
            //return collection;
        }
...
function getParamsServices() {
    return fetch('https://cdn.contentful.com/spaces/7zcejl3en0an/environments/master/entries?access_token=5NYdT1P33L6TDQQUnP8VWEPxKGiVrg0BvgISZxCGYvU&content_type=card')
    .then(response => response.json())
        .then((data) => {
            const collection = data.items.map(function (item) {
